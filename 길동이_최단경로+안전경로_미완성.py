...
def create_chatbot_window():
    ...
    def main():
        ...
        messages = [
            {"role": "system", "content":  'you are a helpful assistant. 입력받은 user_input1에서 user_input2로 걸어서 가는데, 그 사이의 모든 안전한 *동대문구 안의 도로명 주소* 를 알려줘. 형식은 다른 문자열 붙이지 말고 "user_input1, 이문로, 석관로, 왕산로, 회기로, 안암로, user_input2, longitude of user_input1, latitude of user_input1,longitude of 이문로, latitude of 이문로, longitude of 석관로, latitude of 석관로, longitude of 왕산로, latitude of 왕산로, longitude of 회기로, latitude of 회기로, longitude of 안암로, latitude of 안암로,  longitude of user_input2, latitude of user_input2" 예시처럼 순서대로 . longitude of user_input1, latitude of user_input1,longitude of 이문로, latitude of 이문로, longitude of 석관로, latitude of 석관로, longitude of 왕산로, latitude of 왕산로, longitude of 회기로, latitude of 회기로, longitude of 안암로, latitude of 안암로,  longitude of user_input2, latitude of user_input2 이런 것들은 무조건 숫자로. 출력 문자열에 절대 따옴표가 있으면 안돼. '}
        ...
        if user_input1.strip() != "" and user_input2.strip() != "":
            ...
            for item in addresses:
                ...
                try:
                    float(item)
                    coordinates.append(float(item))  # 부동소수점 변환 후 추가
                except ValueError:
                    node_names.append(item.strip())  # 이름 추가
            # addresses에는 이름만 남김
            addresses = node_names


            
            bot_response += f"""\n\n
                "길동이..계산 중.. 출발지의 도로명 주소를 바탕으로 경도, 위도를 계산 중... "
                "출발지({user_input1})의 경도: {coordinates[0]}"
                "출발지({user_input1})의 위도: {coordinates[1]}"
                "길동이..계산 중.. 도착지의 도로명 주소를 바탕으로 경도, 위도를 계산 중... "
                "도착지({user_input2})의 경도: {coordinates[len(coordinates)-2]}"
                "도착지({user_input2})의 위도: {coordinates[len(coordinates)-1]}"\n\n
                "길동이..계산 중.. 두 지점 사이의 안전한 주소 계산...경도, 위도 계산 중... "
                "{coordinates[2:-3]}"
                \n\n\n\n
                "길동이...안전 경로...계산 중..."
                \n
                ...
                \n
                \n
                "길동이: 계산한 좌표를 바탕으로 '서울특별시 동대문구 지도'에 '출발지와 도착지의 안전 경로'를 빨간 선으로 표시했습니다! 팝업된 '서울특별시 동대문구 지도'를 확인해주세요."\n
                "길동이: 팝업된 지도를 전체화면으로 만들고, 돋보기 버튼을 누르시면 안전 경로를 확대해서 자세히 볼 수 있습니다!"
                \n
                \n
                \n
                \n
            ...
            chat_display.insert(tk.END, f"""
                "길동이: 안전 경로, 안전 등급은 잘 보셨나요? 추천된 안전 경로 지도의 빨간 선을 참고해서 목적지까지 갈 때 안전 등급을 참고하면 좋아요. "\n
                "길동이: 안전 등급이 '위험','보통'인 길은 주의하시길 바래요. 지도에서 다른 길로 우회해서 가는 것도 하나의 방법입니다! "\n
            ...
            # 노드 리스트 생성
            nodes = []
            for i in range(0, len(coordinates), 2):  # (x, y) 쌍으로 추출
                lon, lat = float(coordinates[i]), float(coordinates[i + 1])
            ...
            for i in range(len(nodes) - 1):
                ...
                if i == 0:
                    route_full += route_segment  # 첫 번째 경로는 전체 추가
                else:
                    route_full += route_segment[1:]  # 이후 경로는 중복 제거 후 추가


            # 경로 시각화
            fig, ax = ox.plot_graph_route(G, route_full, node_size=0, route_color='red', route_linewidth=2)
